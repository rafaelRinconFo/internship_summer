...
class DMPLoss(nn.Module):
    ...
    def forward(self, endpoints):
        ...
        for s in reversed(range(self.default_params["num_scales"])):
            ...
            translation = torch.add(
                residual_translation, background_translation.view(-1, 1, 1, 3)
            )
            flipped_translation = (
                flipped_residual_translation
                + flipped_background_translation.view(-1, 1, 1, 3)
            )
            ...
            transformed_depth = transform_depth_map.using_motion_vector(
                torch.squeeze(predicted_depth_stack, dim=1),
                translation,
                rotation,
                intrinsics_mat,
                intrinsics_mat_inv,
            ...
            _losses["rgb_consistency"] = (
                _losses["rgb_consistency"] + scale_w * loss_endpoints["rgb_error"]
            )
            _losses["ssim"] = (
                _losses["ssim"] + scale_w * 0.5 * loss_endpoints["ssim_error"]
            )

            _losses["rotation_cycle_consistency"] = _losses[
                "rotation_cycle_consistency"
            ] + (scale_w * loss_endpoints["rotation_error"])
            _losses["translation_cycle_consistency"] = _losses[
                "translation_cycle_consistency"
