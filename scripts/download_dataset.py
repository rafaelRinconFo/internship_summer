    ...
    def delete_zip_files(self, dir: str) -> None:
        for year in self.dataset_years:
            if year not in self.available_years:
                raise ValueError(
                    f"Year {year} is not available. Available years are {self.available_years.keys()}"
                )
            else:
        ...
        for year in self.dataset_years:
            if year not in self.available_years:
                ...
                with zipfile.ZipFile(
                    os.path.join(dir, self.available_years[year]), "r"
                ) as zip_ref:
...
def main():
    parser = argparse.ArgumentParser(
        description="Download dataset from https://www.seanoe.org/data/00810/92226/"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--download_all", type=bool, help="Download all available years", default=False
    )
