...
class SupervisedMidasDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split_csv_file: str, transform=None, toy = False, split = 'train'):
        ...
        self.list_images_paths = [os.path.join(f'datasets/{year}/dense/images', image_name) for year,image_name in zip(self.df['year'],self.df['image_name'])]
        self.list_depth_paths = [os.path.join(f'datasets/{year}/dense/depth', f"depth_{image_name}") for year,image_name in zip(self.df['year'],self.df['image_name'])]
        self.transform= transform
        if toy:
            self.list_images_paths = self.list_images_paths[:int(len(self.list_images_paths)*0.1)]
            self.list_depth_paths = self.list_depth_paths[:int(len(self.list_depth_paths)*0.1)]
