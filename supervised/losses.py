...
class ScaleInvariantLoss(nn.Module):
    ...
    def forward(self, pred, y):
        ...
        n = torch.numel(d)
        loss = torch.sum(d ** 2) / n - self.lambda_ * ((torch.sum(d ** 2)) ** 2) / (
            n ** 2
        )
...
class SSIM(nn.Module):
    ...
    def forward(self, pred, y):
        ...
        for prediction, ground_truth in zip(pred, y):
            ...
            sigma1_sq = (
                F.conv2d(prediction ** 2, self.kernel, padding=self.window_size // 2)
                - mu1_sq
            )
            sigma2_sq = (
                F.conv2d(ground_truth ** 2, self.kernel, padding=self.window_size // 2)
                - mu2_sq
            )
            sigma12 = (
                F.conv2d(
            ...
            structure = (sigma12 + c2 / 2) / (
                torch.sqrt(sigma1_sq * sigma2_sq) + c2 / 2
            )
    ...
    def create_window(self, window_size, channel=1):
        ...
        _1d_window = self.create_gaussian_kernel(
            window_size=window_size, sigma=1.5
        ).unsqueeze(1)
